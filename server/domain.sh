...
WWW=$(dig +short "www.$D" A @8.8.8.8 | tail -1)
if [ "$IP" != "147.45.141.23" ]; then
  echo "Голый домен $D сейчас показывает на «$IP», а нужно 147.45.141.23."
  [ "$WWW" = "147.45.141.23" ] && echo "При этом www.$D уже настроен верно — значит, не хватает записи для самого домена (имя «@» или пустое)."
  echo "Поправьте А-запись у регистратора и подождите (обычно 15–60 минут)."
  exit 1
fi
[ "$WWW" = "147.45.141.23" ] || echo "Замечание: www.$D показывает на «$WWW» — с него переадресации не будет."
