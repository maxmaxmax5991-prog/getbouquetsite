...
cronAdd("ms-status", "* * * * *", () => {
  ...
  // Порядок важен: побеждает первое совпадение, поэтому от конечных состояний к начальным.
  // «Принят, Не оплачен» раньше не попадал никуда, и заказ навсегда оставался «Новым».
  const MAP = [
    [/отмен|отклонен|возврат|утилизирован/i, "cancelled"],
    [/самовывоз завершен|завершен|доставлен|выполнен/i, "done"],
    [/доставля|в пути|передан службе доставки|курьер/i, "delivering"],
    [/готов к самовывозу|собран|ожидает отправку|ожидает отгрузк/i, "photo"],
    [/сборка|ожидает сборку|комплектовать/i, "assembling"],
    [/принят|подтвержден|можно комплектовать|оплачен|платеж авторизован/i, "confirmed"],
  ];
